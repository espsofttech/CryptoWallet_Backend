...
class withdrawalModel{
    ...
    getAllList= async()=>{
        let sql = `SELECT withdrawList.user_id,withdrawList.coin_id,withdrawList.id,withdrawList.createdAt,withdrawList.status,withdrawList.withdrawal_Address,withdrawList.amount,withdrawList.remark,withdrawList.transaction_image,users.first_name AS userName,coins.coinName FROM withdrawList LEFT JOIN users ON withdrawList.user_id = users.id LEFT JOIN coins ON withdrawList.coin_id= coins.id ORDER BY withdrawList.id DESC`;
        const [result, fields] = await promisePool.query(sql);
        return result;
    }
    ...
    updateTransactionStatus = async(data)=>{
        console.log("Asdf",data);
        // SET status = '${data.type}' ,
        let sql = `UPDATE withdrawList set Status = '${data.type}' , transaction_image = '${data.transactionImage}', remark ='${data.remark}' WHERE id ='${data.id}'`;
        const [result, fields] = await promisePool.query(sql);
        return result;
    } 
