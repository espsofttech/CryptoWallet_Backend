...
class bankModel {
  ...
  checkAdminByid = async (id) => {
    let sql = `SELECT * FROM  admin_bank WHERE id='${id}'`;
    const [result, fields] = await promisePool.query(sql);
  ...
  deleteAdminDetails = async (id) => {
    let sql = `DELETE FROM admin_bank WHERE id ='${id}'`;
    const [result, fields] = await promisePool.query(sql);
    return result;
  };
  ...
  insertDetails = async (data) => {
    let sql = `INSERT INTO admin_bank(user_id,bank_account_holder_name,bank_name,branchName,AccountNumber,ifsc_code,crypto_address,type,crypto_type)VALUES('${data.user_id}','${data.bank_account_holder_name}','${data.bank_name}','${data.branchName}','${data.AccountNumber}','${data.ifsc_code}','${data.crypto_address}','${data.type}','${data.crypto_type}')`;
    const [result, fields] = await promisePool.query(sql);
    return result;
  };
  ...
  getDetails = async () => {
    let sql = `SELECT * FROM admin_bank order by id desc`;
    const [result, fields] = await promisePool.query(sql);
    return result;
  };
