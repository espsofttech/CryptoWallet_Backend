...
const updateTransactionStatus = async (req, res) => {
  try {
    let transactionImage = !req.files["transaction_image"] ? null : req.files["transaction_image"][0].filename;
    if (transactionImage) {
      req.body.transactionImage = transactionImage;
    } 
    // console.log(req.body);
    const updateTransactionStatus = await withdrawalModel.updateTransactionStatus(req.body);
    if (updateTransactionStatus) {
      return res.status(200).send({ status: true, msg: "status updated" });
    } else {
      return res.status(400).send({
        status: false,
        msg: "Something went wrong",
