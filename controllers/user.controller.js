...
const getUserdDetailById = async (req, res) => {
  try {
    console.log("asdf");
    console.log("12346",req.body);
    const id = req.params.id;

    const checkId = await userModel.FindById(req.body.id);
    if (checkId) {
      const getDetails = await userModel.getUsersById(req.body.id);
